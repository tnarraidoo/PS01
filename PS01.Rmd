...
f3 <- as.data.frame(cbind(Variable,Description))
...
We see that around 74% of women survived while abour 81% of men survived. However, we also need to consider age. For the purpose of our analysis we will consider passengers under 18 as children. To get a better picture, we could look at the age and gender of those that survived or died.
...
#Apply our model on the train set to look at error rates.
train <- train %>%
  mutate(Survived_train_pred = ifelse(Sex=="female", sample(y,1),ifelse(!is.na(Age) & Age <= 10,1,sample(x,1)))) 

table(train$Survived_train_pred,train$Survived)
```

From our first model, we see we are able to correctly predict those that died in 83% of cases and those that survived in 74% of cases using the train set.

```{r}
#extract PassengerId and Survived variables for submission.
pred1 <- pred1 %>%
